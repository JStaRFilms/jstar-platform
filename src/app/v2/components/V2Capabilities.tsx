        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
          {PILLARS.map((pillar) => {
                  ...
                  <div className="h-full flex flex-col justify-between space-y-8">
                    <div className="space-y-6">
                      <div className="flex items-center justify-between text-xs font-mono">
                        <span className="text-chartreuse font-bold">{`[${pillar.num} // CORE DIVISION]`}</span>
                        <span className="px-2 py-0.5 rounded bg-chartreuse/10 text-chartreuse text-[10px] border border-chartreuse/30">
                          FEATURED
                        </span>
                      </div>
                    ...
                    <a
                      href="#work"
                      className="pt-4 border-t border-powder-blue/10 flex items-center justify-between text-xs font-mono text-chartreuse font-bold hover:underline"
                    >
                      <span>Explore Labs Deployments</span>
            ...
            return (
              <div
                key={pillar.num}
                className="p-8 sm:p-10 rounded-3xl bg-card border border-powder-blue/15 flex flex-col justify-between space-y-8 hover:border-chartreuse/40 transition-colors group shadow-xl"
              >
                <div className="space-y-6">
                  <div className="flex items-center justify-between text-xs font-mono text-powder-blue/60">
                    <span>{`[${pillar.num} // DIVISION]`}</span>
                    <span className="text-[10px]">ACTIVE</span>
                  </div>

                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                    pillar.num === '01' ? 'bg-toffee-brown/20 border border-toffee-brown/40 text-toffee-brown' : 'bg-powder-blue/10 border border-powder-blue/30 text-powder-blue'
                  }`}>
                    <Icon className="w-6 h-6" />
                  </div>
                ...
                <a
                  href="#work"
                  className="pt-4 border-t border-powder-blue/10 flex items-center justify-between text-xs font-mono text-powder-blue/70 group-hover:text-ghost-white transition-colors"
                >
